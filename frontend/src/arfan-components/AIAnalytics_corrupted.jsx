...
function AIAnalytics({ open, onClose }) {
  ...
  const getAIRecommendations = async (analysis) => {
    try {
      ...
      const response = await fetch('/api/genai/custom', {
        method: 'POST',
  ...
  const getAIRecommendations = async (analysis, users) => {
    try {
      // For now, skip the API call and use fallback recommendations due to OpenAI API key not configured
      console.log('Using fallback recommendations - OpenAI API key needs to be configured');
      const fallbackRecommendations = generateFallbackRecommendations(analysis);
      return fallbackRecommendations;
    } catch (error) {
      console.error('AI Recommendations error:', error);
