...
function applyMapping(rows, mapping) {
  ...
  if (suggested) {
    // Fallback: map "client_name" → "fullName"
    if (excelHeader === 'client_name' && suggested !== 'fullName') {
      row['fullName'] = originalRow[excelHeader];
    } else {
...
// Extra fallback if mapping missed it entirely:
if (!row.fullName && originalRow['client_name']) {
  row.fullName = originalRow['client_name'];
}
